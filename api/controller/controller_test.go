...
func TestGetPhotosExifInfo(t *testing.T) {
	...
	apiName := "/view/getImagesExifInfo"
	gin.SetMode(gin.TestMode)
	gin.DefaultWriter = os.Stdout
	// 初始化Gin引擎
	router := gin.New()
	// 注册GET路由
	router.POST(apiName, GetPhotosExifInfo)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(tc.method, apiName, strings.NewReader(values.Encode()))
			if err != nil {
				t.Fatalf("构建请求失败, err: %v", err)
			}
			// 关键点3， 设置请求头，一定
			req.Header.Set("Content-Type", tc.reqType)
			// 构造一个记录
			...
			// 检查响应码
			assert.Equal(t, tc.wantCode, result.StatusCode)
...
func TestShowImage(t *testing.T) {
	...
	apiName := "/view/showImage"
	gin.SetMode(gin.TestMode)
	gin.DefaultWriter = os.Stdout
	// 初始化Gin引擎
	router := gin.New()
	// 注册GET路由
	router.GET(apiName, ShowImage)
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			values := make(url.Values)
			for k, v := range tc.reqBody {
...
func TestExifInfoExportBySaveFile(t *testing.T) {
	savePath := internal.GetRuntimePath("export-1.csv")
	// 定义测试用例
	testCases := []struct {
		name     string
		method   string
	...
	apiName := "/view/exifInfoExportv2"
	gin.SetMode(gin.TestMode)
	gin.DefaultWriter = os.Stdout
	// 初始化Gin引擎
	router := gin.New()
	// 注册GET路由
	router.POST(apiName, ExifInfoExportBySaveFile)
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			values := make(url.Values)
			for k, v := range tc.reqBody {
...
func TestShowPhotoFrameParams(t *testing.T) {
	...
	apiName := "/frame/showPhotoFrame"
	gin.SetMode(gin.TestMode)
	gin.DefaultWriter = os.Stdout
	// 初始化Gin引擎
	router := gin.New()
	// 注册GET路由
	router.POST(apiName, ShowPhotoFrame)
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			values := make(url.Values)
			for k, v := range tc.reqBody {
...
func TestShowPhotoFrameFeature(t *testing.T) {
	...
	if pkg.HasError(err) {
		t.Errorf("ReloadFrameTemplate failed, err: %v", err)
	}
	...
	for key := range frame.GetTemplateInfo() {
		for i, name := range []string{"/tests/1.jpg", "/tests/2.jpg"} {
			t.Run(key+fmt.Sprintf("%d_2", i+1), func(t *testing.T) {
				savePath := internal.GetRootPath() + "/tests/" + key + "_" + fmt.Sprintf("%d", i+1) + "_2.jpg"
				...
				req, err := http.NewRequest(http.MethodPost, apiName, strings.NewReader(values.Encode()))
				if err != nil {
					t.Fatalf("构建请求失败, err: %v", err)
				}
				// 关键点3， 设置请求头，一定
